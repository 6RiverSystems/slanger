    ...
    def run
      ...
      EM.run do
        ...
        EM::WebSocket.start options do |ws|
          ...
          ws.onopen        { |handshake|
            puts "Slanger:WebSocketServer:onopen #{handshake}"
            ws.connection_handler = Slanger::Config.socket_handler.new ws, handshake
          }
          ws.onmessage     { |msg|
            puts "Slanger:WebSocketServer:onmessage #{msg}"
            ws.connection_handler.onmessage msg
          }
          ws.onclose       {
            puts "Slanger:WebSocketServer:onclose"
